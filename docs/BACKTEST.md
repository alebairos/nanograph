...
**G48 (LLVM BOLT cmp_order, done).** First ordering-bug backtest. llvm/llvm-project BOLT `compareSections`, parent `e8606ab` to fix `5fe235b`. The missing `A == B` guard breaks **irreflexivity** for every section whose self-comparison reaches a `return true` path (mover, main, warm in the modeled scenario; cold compares equal names and stays 0). New `cmp_order` relation (irreflexivity + antisymmetry; only irreflexivity has a tested witness). `fixtures/metamorphic/llvm_bolt_cmp.c` transcribes the string ordering logic; strippable identity check, rev2 `-DCMP_IDENTITY_OK`. Full 4x4 probe matrix. **Catch** on witness `hex=00` (pair 0,0, first in probe order): buggy `got_ij=1`, honest `0`. Timeline accept, reject, accept; gated `LLVM-BOLT-CMP`. See `fixtures/backtest/llvm-bolt-cmp/CASE.md`. Scored in `fixtures/fit-cases/llvm-bolt-cmp-order.fit`.

**G42 (cosmo ljson ICP, done).** Reselected real-history follow-through after punycode (CVE-2022-3602) proved a memory-safety OOB that `round_trip` cannot observe. cosmopolitan `ljson` string decode, parent `ccd057a` to fix `baf51a4` ("Add utf-8 validation to ljson"). The pre-fix loop copied string bytes above `0x1F` verbatim; the fix adds the `kJsonStr` classifier and rejects overlong, surrogate, and malformed UTF-8. `fixtures/metamorphic/cosmo_ljson.c` transcribes the classifier and raw-byte validation behind a trusted driver, strippable honest decoder, rev2 compiled with `-DLJSON_NOUTF8` for the verbatim parent path. The relation is `value_oracle`, not the scorecard's `round_trip`: a verbatim pass-through makes `round_trip` the identity, so it misses the bug; the acceptance gap needs `value_oracle`. **Catch** on `c080` (overlong U+0000): buggy echoes `c080`, honest rejects. Timeline accept, reject, accept; gated `COSMO-LJSON`. See `fixtures/backtest/cosmo-ljson/CASE.md`. Scored in `fixtures/fit-cases/cosmo-ljson-overlong.fit`.
