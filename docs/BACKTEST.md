...
**G35 (real Knuth canon, done).** The first backtest on Knuth's actual canon. The documented `rand_len` off-by-one in `gb_rand.w`: buggy `min_len+gb_unif_rand(max_len-min_len)` never reaches `max`; the fix adds `+1`. Mirror `ascherer/sgb` `fd99287` (buggy) to `65433e2` (fixed). `fixtures/metamorphic/knuth_rand_len.c` vendors Knuth's `gb_flip` + `rand_len`, transcribed at `fd99287`, validated against his `test_flip` constants (`gb_init_rand(-314159)` then `gb_next_rand()==119318998`). Runs his actual arithmetic, no simulation. The new `range_coverage` relation sweeps gb_flip seeds and asserts the observed `[min,max]` equals the declared `[lo,hi]`; the driver is a pure generator, the verifier aggregates. **Catch** over 256 seeds: honest `[1,10]` accepts, buggy `[1,9]` rejects, witness `hex=09`. Timeline accept, reject, accept; gated `KNUTH-RAND-LEN`. See `fixtures/backtest/knuth-rand-len/CASE.md`.
