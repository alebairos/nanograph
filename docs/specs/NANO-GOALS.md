...
| G85 | Native-upstream hunt vehicle (relation on live code, not transcription) | #126 | n/a | **Active** (`native-hunt.sh` + self-test; CPython base64 reproduces tranche-1 `relation_gap` transcription-free; bitcoin prep done: real `rust-bitcoin` `VarInt` canonical-enforcing target `verdict=accept`, negative control proves probes catch a lenient codec; honest null at HEAD; next is a thinner Bitcoin-adjacent codec where a reject is a candidate defect) |
