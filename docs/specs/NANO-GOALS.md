...
| Differential relation reproduces a pinned historical high-severity defect | **Proven (G88, base-x CVE-2025-27611)** | pinned `base-x@5.0.0` target accepts Unicode homoglyph witness `ABCĀDEF` (`target_out=50f12020b0`) while strict Base58 reference rejects; asymmetry classifies `reason=target_accepts_reference_rejects`; pinned fixed comparator `base-x@5.0.1` concurs and accepts run; gated `check-basex-homoglyph-hunt.sh`; logs persisted in `.harness-data/agent-eval/g84-pinned-repro/`; G89+G90 extend queue; aggregate `check-pinned-historical-repros.sh`; [`PROBE-GENERATOR-SPIKE.md`](PROBE-GENERATOR-SPIKE.md) |
| Differential relation finds a novel defect on unaudited upstream (no pre-existing issue) | **Proven (G91, pbech32 0.2.0)** | thin single-maintainer Rust codec accepts a 109-char valid-checksum bech32m string (`bc1qqqq…hajhhf`) the sipa BIP173 reference rejects on the 90-char cap; `reason=target_accepts_reference_rejects`; checksum-tamper rejects so it is a missing length cap not blind accept; short valid strings concur byte-for-byte; severity low, plausibly intentional relaxation; gated `check-pbech32-hunt.sh`; logs in `.harness-data/agent-eval/g91-novel-pbech32/`; [`PROBE-GENERATOR-SPIKE.md`](PROBE-GENERATOR-SPIKE.md) |
...
| G84 | Blind discovery on real upstream (reject nobody flagged) | #126 | n/a | **Partial** (two novel rejects, both low severity: G86 `1200wd/bitcoinlib` CompactSize #127, G91 `pbech32` missing BIP173 length cap; G88 historical CVE repro proves vehicle credibility; maintained PyPI tier null twice; novel high-severity yield likely needs thin GitHub-only Go targets, blocked on toolchain) |
...
| G88 | Pinned historical repro: base-x CVE-2025-27611 | #126 | n/a | **Done** (`base-x@5.0.0` target wired with Node CLI; differential witness `ABCĀDEF` causes defect-direction reject; fixed `base-x@5.0.1` comparator concurs; gated `check-basex-homoglyph-hunt.sh`; issue-scoped logs in `.harness-data/agent-eval/g84-pinned-repro/`) |
| G89 | Pinned historical repro: rust-bech32 #274 | #126 | n/a | **Done** (`bech32@0.12.0` `validate_segwit` path accepts witver 17; `SegwitHrpstring::new` fixed comparator; witness `bc13...`; gated `check-rust-bech32-hunt.sh`) |
| G90 | Pinned historical repro: Base58Swift #23 | #126 | n/a | **Done** (Base58Swift 2.1.x under-decodes leading zero; PR #21 head fixed comparator; witness `16L5yRNPTuciSgXGHqYwn9N6NeoKqopAu`; gated `check-base58swift-hunt.sh`; aggregate `check-pinned-historical-repros.sh`) |
| G91 | Novel discovery: pbech32 missing BIP173 length cap | #126 | n/a | **Done** (first novel reject from the novel-discovery queue with no pre-existing issue; `pbech32` 0.2.0 accepts a >90-char valid-checksum bech32m string the sipa reference rejects; witness `bc1qqqq…hajhhf`; checksum-tamper rejects so it is a missing length cap not blind accept; severity low; gated `check-pbech32-hunt.sh`; maintained PyPI tier null again) |
