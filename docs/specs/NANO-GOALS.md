...
| Native hunt finds real upstream defect without transcription | **Proven (G86, bitcoinlib)** | `1200wd/bitcoinlib` CompactSize at `bec99a2` via `native-hunt.sh`; `verdict=reject` witness `fdffff` (decode `65535`, reencode `feffff0000`, canonical wire input); encoder uses `< 0xffff` not `<=`; decoder lacks minimality check; gated `check-bitcoinlib-compactsize-hunt.sh`; ADR-023; [`fixtures/backtest/bitcoinlib-compactsize/CASE.md`](../fixtures/backtest/bitcoinlib-compactsize/CASE.md) |
...
| G86 | First native upstream defect (1200wd/bitcoinlib CompactSize) | #126 | n/a | **Done** (`verdict=reject` witness `fdffff`; gated `check-bitcoinlib-compactsize-hunt.sh`; CASE + ADR-023; maintainer report draft, no external issue opened) |
