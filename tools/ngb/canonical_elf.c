...
size_t ngb_canonical_elf_build_segment(const uint8_t *code, size_t code_len,
  ...
  const uint32_t ph_num = 1;
  const uint32_t ph_entsize = 56;
  const uint64_t seg_off = (uint64_t)ph_off + (uint64_t)ph_num * ph_entsize;
  const size_t seg_len = code_len + data_len;
  const size_t total = 64 + 56 + seg_len;
  ...
  uint32_t flags = 5;
  memcpy(ph + 4, &flags, 4);
  uint64_t seg_vaddr = e_entry;
  memcpy(ph + 8, &seg_off, 8);
  memcpy(ph + 16, &seg_vaddr, 8);
  memcpy(ph + 24, &seg_off, 8);
  uint64_t filesz = seg_len;
  memcpy(ph + 32, &filesz, 8);
  memcpy(ph + 40, &filesz, 8);
  uint64_t align = 0x1000;
  memcpy(ph + 48, &align, 8);

  memcpy(out + (size_t)seg_off, code, code_len);
  if (data_len > 0)
    memcpy(out + (size_t)seg_off + code_len, data, data_len);
  return total;
}
