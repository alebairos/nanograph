...
static const NgbNodeSpec ADD_TWO_NODES[] = {
    {1, 0, 5},
    {2, 5, 3},
    {3, 8, 9},
};
...
int main(int argc, char **argv) {
  ...
  uint8_t elf[256];
  size_t elf_len = ngb_add_two_elf_build(elf, sizeof(elf));
  if (elf_len == 0) {
    fprintf(stderr, "add-two-fixture: elf build failed\n");
    return 1;
  }
  ...
  size_t ngb_len = 0;
  NgbStatus st =
      ngb_pack_elf_nodes(elf, elf_len, NGB_ARCH_X86_64_LINUX_ELF, ADD_TWO_NODES,
                         3, &ngb, &ngb_len);
  if (st != NGB_OK) {
    fprintf(stderr, "add-two-fixture: %s\n", ngb_status_str(st));
    return 1;
  }
  ...
  if (write_fixtures) {
    ...
    char path[1200];
    snprintf(path, sizeof(path), "%s/add_two_elf.bin", dir);
    if (write_bytes(path, elf, elf_len) != 0)
      return 1;
    snprintf(path, sizeof(path), "%s/add_two.ngb", dir);
    if (write_bytes(path, ngb, ngb_len) != 0)
      return 1;
    snprintf(path, sizeof(path), "%s/add_two.ngb.hex", dir);
