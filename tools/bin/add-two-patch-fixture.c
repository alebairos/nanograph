...
#define ADD_TWO_PATCH_DELTA_OFF 127
#define ADD_TWO_PATCH_ID 1
#define ADD_TWO_PATCH_TS 1700000000ULL
...
int main(int argc, char **argv) {
  ...
  if (read_bytes(genesis_path, &genesis, &genesis_len) != 0) {
    fprintf(stderr, "add-two-patch-fixture: read %s failed\n", genesis_path);
    return 1;
  }

  static const uint8_t delta[] = {0x01, 0x02};
  NgbPatchInput patch = {
      .patch_id = ADD_TWO_PATCH_ID,
      .delta_off = ADD_TWO_PATCH_DELTA_OFF,
      .delta_len = 2,
      .delta = delta,
      .timestamp = ADD_TWO_PATCH_TS,
  };
  ...
  free(genesis);
  if (st != NGB_OK) {
    fprintf(stderr, "add-two-patch-fixture: %s\n", ngb_status_str(st));
    return 1;
  }
  ...
  if (write_fixtures) {
    ...
    char path[1200];
    snprintf(path, sizeof(path), "%s/add_two_patched.ngb", dir);
