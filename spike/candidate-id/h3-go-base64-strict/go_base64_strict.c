 ...
 *
 * C transcription of golang/go encoding/base64 StrictEncoding (issue #15656).
 * Parent 56b5546 decoded the final partial group without checking unused
 * sextet bits; fix 87b1aaa rejects non-zero trailing padding bits so
 * iYV= fails while canonical iYU= accepts.
 *
...
static int decode_block(const char *in, int len, unsigned char *out, dec_state *st) {
  const char *p = in;
...
static int b64_decode(const char *in, int len, unsigned char *out, int cap, int *err) {
  dec_state st = {0, 0, step_a, 0};
  int n = decode_block(in, len, out, &st);
  if (n > cap)
    return -1;
#if !defined(LAX_PAD_BITS)
  if (st.step == step_d && st.plainchar != 0)
    st.had_errors = 1;
  if (st.step == step_c && st.n_padding == 2 && st.plainchar != 0)
    st.had_errors = 1;
#endif
  *err = st.had_errors;
  return n;
